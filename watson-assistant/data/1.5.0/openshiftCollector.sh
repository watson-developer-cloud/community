...
collectDiagnosticsData() {
  ...
  runCommand "cpd-cli status --namespace $namespace" "Check the status of the assembly and modules"
...
echo -e "\n"
echo "**********************************************************"
echo "Checking for Helm/cpd/oc Client"
echo "**********************************************************"
if ! which $oc; then
...
if ! which cpd-cli; then
  echo "CPD client not found. Ensure that you have cpd-cli installed and on your PATH."
  exit 1
fi
...
$oc get secret helm-secret -n $namespace >/dev/null 2>&1
SECRET_RC=$?
