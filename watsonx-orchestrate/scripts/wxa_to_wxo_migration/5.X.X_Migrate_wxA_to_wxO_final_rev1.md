...
### 5.1 Extract Instance Metadata Using Script (Alternative Method)

* Alternatively, you can use the [`wxa_extract_instances.sh`](wxa_extract_instances.sh) script to automatically extract instance metadata and generate files for migration.

#### 5.1.1 Run the Extraction Script

```bash
./wxa_extract_instances.sh <wxA-namespace>
```

Or with a custom output filename:

```bash
./wxa_extract_instances.sh <wxA-namespace> my_migration
```

#### 5.1.2 Review Generated Files

The script generates two files:

* **`wxa_instances.csv`** - CSV format for creating wxO instances (compatible with [`instancecreation_v2.sh`](instancecreation_v2.sh))
  * Format: `id,service,version`
  * Example: `instance-1,orchestrate,5.2.2`

* **`wxa_instances.json`** - JSON format with full metadata including tenant_id for mapping generation
  * Contains: `wxa_tenant_id`, `wxa_instance_id`, `wxa_name`, `wxa_region`, `wxo_name`

#### 5.1.3 Verify the Output

```bash
cat wxa_instances.csv
cat wxa_instances.json
```

> **Note:** The script automatically:
> - Filters out deleted instances
> - Handles monitoring and bdd-test instances
> - Skips System region instances
> - Orders instances with monitoring first
...
#### 13.1 Create Instances Using Script (Automated Method)

* Alternatively, you can use the [`instancecreation_v2.sh`](instancecreation_v2.sh) script to automatically create wxO instances from the CSV file generated in step 5.1.

```bash
./instancecreation_v2.sh wxa_instances.csv <NAMESPACE> <CPD_ROUTE> <API_KEY> <USERNAME>
```

**Example:**
```bash
./instancecreation_v2.sh wxa_instances.csv cpd https://cpd.example.com <your-api-key> cpadmin
```

**Parameters:**
- `wxa_instances.csv` - CSV file generated from step 5.1
- `<NAMESPACE>` - Namespace where service instances will be created
- `<CPD_ROUTE>` - CPD route (e.g., cpd.example.com or https://cpd.example.com)
- `<API_KEY>` - Your CPD API key
- `<USERNAME>` - Your CPD username

> **Note:** The script will create instances sequentially and wait for each instance to be provisioned before proceeding to the next one.

#### 13.2 Extract wxO Instance Information

After creating the wxO instances, extract their information using the [`wxo_extract_instances.sh`](wxo_extract_instances.sh) script:
...
This will generate a JSON file with instance names and tenant_ids needed for the migration mapping.
...
[
  {
    ...
    "instance_id": "1764035503433645",
    "region": "us-south"
  }
]
```

> **Note:** This step extracts the wxO instance tenant_ids from the PostgreSQL database, which are required for creating the pgmig mapping file. The script requires oc CLI access to the wxO cluster.

#### 13.3 Generate Instance Mapping File

After extracting both wxA and wxO instance information, generate the mapping file using the [`wxa_generate_mapping.sh`](wxa_generate_mapping.sh) script:

```bash
./wxa_generate_mapping.sh wxa_instances.json wxo_instances.json
```

This will generate an `enteredMapping.yaml` file that maps wxA tenant_ids to wxO tenant_ids.
...
**Verify the mapping:**
```bash
cat enteredMapping.yaml
```

> **Note:** This mapping file will be used with the pgmig tool in step 27 to ensure data is migrated to the correct wxO instances. Review the mapping carefully to ensure each wxA instance is mapped to the correct wxO instance by name.

#### 13.4 Verify Assistant Builder Access

After creating the wxO instances, verify that the Assistant Builder is accessible in one of the instances:

1. **Access the CPD UI**
   - Navigate to your Cloud Pak for Data console
   - Go to **Services** → **Instances**

2. **Open a wxO Instance**
   - Select one of the newly created wxO instances (e.g., `instance-1`)
   - Click **Open** to launch the instance

3. **Verify Assistant Builder**
   - In the wxO instance, look for the **Assistant Builder** option in the navigation menu
   - Click on **Assistant Builder** to open it
   - Verify that the interface loads correctly

4. **Check for Errors**
   - Ensure there are no error messages
   - Verify that you can see the assistant creation interface
   - Confirm that all UI elements are rendering properly

> **Note:** If Assistant Builder is not visible or accessible, ensure that the instance has been fully provisioned (status should be "PROVISIONED" or "Ready"). You may need to wait a few minutes after instance creation for all services to be fully initialized.

> **Troubleshooting:** If Assistant Builder is not available:
> - Check the instance status in the CPD UI
> - Verify that the wxO operator is running: `oc get pods -n <operator-namespace> | grep watson-orchestrate`
> - Check the instance logs for any errors: `oc logs -n <namespace> <instance-pod-name>`
