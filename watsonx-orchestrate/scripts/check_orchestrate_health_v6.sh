...
#
# USAGE:
#  ./check_orchestrate_health_v6.sh [OPTIONS]
#
#  Options:
...
print_header_details_box() {
  ...
  echo "╔══════════════════════════════════════════════════════════════════════════════╗"
  print_box_line "                watsonx Orchestrate Configuration Details"
  echo "╠══════════════════════════════════════════════════════════════════════════════╣"
  print_box_blank
...
_get_component_container_resources() {
  ...
  cpu_req=$($OC -n "$ns" get "$kind" "$cname" \
    -o jsonpath="{.spec.template.spec.containers[$container_index].resources.requests.cpu}" 2>/dev/null || echo "")
  mem_req=$($OC -n "$ns" get "$kind" "$cname" \
    -o jsonpath="{.spec.template.spec.containers[$container_index].resources.requests.memory}" 2>/dev/null || echo "")
  cpu_lim=$($OC -n "$ns" get "$kind" "$cname" \
    -o jsonpath="{.spec.template.spec.containers[$container_index].resources.limits.cpu}" 2>/dev/null || echo "")
  mem_lim=$($OC -n "$ns" get "$kind" "$cname" \
    -o jsonpath="{.spec.template.spec.containers[$container_index].resources.limits.memory}" 2>/dev/null || echo "")
  ...
  $OC -n "$ns" get "$kind" "$cname" -o json 2>/dev/null | \
    jq -r '
      ...
      [
        ...
        .value.name,
        (.value.image // ""),
        (.value.resources.requests.cpu // "not set"),
        (.value.resources.requests.memory // "not set"),
        (.value.resources.limits.cpu // "not set"),
        (.value.resources.limits.memory // "not set")
      ] | @tsv
    ' > "$out_file" 2>/dev/null || :
}
...
modify_component_sizing() {
  ...
  case "$sizing_top" in
    ...
    2)
      ...
      # Show all containers and let user pick which one to modify
      echo ""
      echo "Fetching containers for $sm_key_2..."
      local tmp_containers selected_container_index selected_container_name
      ...
      if [ ! -s "$tmp_containers" ]; then
        echo "â No containers found for $selected_name2"
        rm -f "$tmp_containers"
        return
      fi
      ...
      echo ""
      printf "%-4s %-35s %-14s %-14s %-14s %-14s\n" "#" "Container" "CPU Req" "CPU Lim" "Mem Req" "Mem Lim"
      printf "%-4s %-35s %-14s %-14s %-14s %-14s\n" "---" "-----------------------------------" "--------------" "--------------" "--------------" "--------------"
      while read -r line; do
        num=$(echo "$line" | cut -f1)
        idx=$(echo "$line" | cut -f2)
        cname=$(echo "$line" | cut -f3)
        cpu_req_live=$(echo "$line" | cut -f5)
        mem_req_live=$(echo "$line" | cut -f6)
        cpu_lim_live=$(echo "$line" | cut -f7)
        mem_lim_live=$(echo "$line" | cut -f8)
        printf "%-4s %-35s %-14s %-14s %-14s %-14s\n" "$num)" "$cname" "$cpu_req_live" "$cpu_lim_live" "$mem_req_live" "$mem_lim_live"
      done < "$tmp_containers"

      echo ""
      if [ "$container_count" -eq 1 ]; then
        selected_container_index=0
        selected_container_name=$(awk -F'\t' 'NR==1{print $3}' "$tmp_containers")
        echo "Only one container found: $selected_container_name"
      else
        ...
        selected_container_index=$(awk -v n="$cont_choice" 'NR==n{print $2}' FS='\t' "$tmp_containers")
        selected_container_name=$(awk -v n="$cont_choice" 'NR==n{print $3}' FS='\t' "$tmp_containers")
        if [ -z "$selected_container_index" ]; then
          echo "❌ Invalid selection."
          rm -f "$tmp_containers"
          return
        fi
      ...
      # Get current resource values for selected container
      local res_info
      res_info=$(_get_component_container_resources "$ns" "$selected_name2" "$selected_container_index")
      ...
      echo ""
      echo "Container: $selected_container_name"
      ...
      echo "Updating resources for $sm_key_2 (container: $selected_container_name)..."
      $OC -n "$ns" patch wo "$wo_name" --type=merge \
...
\t' read -r num idx cname image cpu_req_live mem_req_live cpu_lim_live mem_lim_live; do
        printf "%-4s %-35s %-14s %-14s %-14s %-14s\n" "$num)" "$cname" "$cpu_req_live" "$cpu_lim_live" "$mem_req_live" "$mem_lim_live"
      done < "$tmp_containers"
...
\t' 'NR==1{print $3}' "$tmp_containers")
        echo "Only one container found: $selected_container_name"
      else
        printf "Select container number (or 'cancel'): "
        read -r cont_choice
        [ "$cont_choice" = "cancel" ] || [ -z "$cont_choice" ] && { echo "Skipped."; rm -f "$tmp_containers"; return; }
        selected_container_index=$(awk -F
...
\t' -v n="$cont_choice" 'NR==n{print $2}' "$tmp_containers")
        selected_container_name=$(awk -F
...
\t' -v n="$cont_choice" 'NR==n{print $3}' "$tmp_containers")
        if [ -z "$selected_container_index" ]; then
      ...
      # Fetch sizeMapping overrides from WO CR for this container
      local sm_cpu_req sm_mem_req sm_cpu_lim sm_mem_lim
