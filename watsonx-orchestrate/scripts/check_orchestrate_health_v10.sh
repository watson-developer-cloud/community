...
#
# USAGE:
#  ./check_orchestrate_health_v9.sh [OPTIONS]
#
#  Options:
...
_SCALABLE_COMPONENTS="agentic-task-manager agent-gateway ai-gateway ai-cognitive-mapper appconnect-skill-provider archer-server archer_de_client_mapper audit_logging automation-discovery builder-ui channel-integrations connection-manager connector-service conversation_controller_mapper data_exhaust_producer_mapper de-client de-seeder de-server discover-skills discover-zos-adapter ibm_connectivity_pack jaeger-collector jaeger-query kafka kafka-operator kafka-zookeeper landing-page mcp-context-forge multi-skill-orchestration-ai openapi-provider opensearch pgbouncer platform-ui postgres rabbitmq rabbitmq-init-container skill-catalog-ui skill-sequencing skill-server socket_handler studio teams-server teams-ui tenant-controller tenant-registry tools-runtime tools-runtime-manager tools-runtime-scheduler uiproxy voice-controller wxdengine_etcd wxo-connections wxo-connections-ui wxo-docker-proxy wxo_chat_client"
...
run_configuration_mode() {
  ...
  while true; do
    ...
    echo "  8. Refresh current configuration"
    echo "  9. Exit configuration mode"
    echo ""
